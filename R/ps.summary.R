...
if(estimand=="ATT")
{
   ...
   if(get.ks)
   {
      ...
      pval_calc <- NSM3::pKolSmirn(as.integer(ess[2])
                                   ,as.integer(ess[1]))
      # pval <- 1 - .C("psmirnov2x", p = as.double(ks),
      #         as.integer(ess[2]), as.integer(ess[1]), PACKAGE = "parallelTwang")$p
      
      pval <- 1 - pval_calc$p.val
...
#######
    ...
    if (get.ks) {
        ...
        #     as.integer(ess[1]), PACKAGE = "parallelTwang")$p
